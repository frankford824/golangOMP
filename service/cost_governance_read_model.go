...
func buildTaskGovernanceReadModels(
...
) (*domain.TaskMatchedRuleGovernance, *domain.TaskCostOverrideSummary, *domain.TaskGovernanceAuditSummary, *domain.AppError) {
	overrideSummary, governanceAuditSummary := buildTaskCostOverrideReadModels(detail, events, overrideEvents)
	if detail == nil {
		return nil, overrideSummary, governanceAuditSummary, nil
	}
	...
	if costRuleRepo == nil || detail.CostRuleID == nil || *detail.CostRuleID == 0 {
		...
		return &domain.TaskMatchedRuleGovernance{MatchedRule: snapshot}, overrideSummary, governanceAuditSummary, nil
	}

	loader := newCostRuleLineageLoader(costRuleRepo, time.Now().UTC())
	matchedRule, err := loader.get(ctx, *detail.CostRuleID)
	if err != nil {
		return nil, overrideSummary, governanceAuditSummary, infraError("get matched cost rule governance", err)
	}
	if matchedRule == nil {
		return &domain.TaskMatchedRuleGovernance{MatchedRule: snapshot}, overrideSummary, governanceAuditSummary, nil
	}
	...
	if err != nil {
		return nil, overrideSummary, governanceAuditSummary, infraError("decorate matched cost rule governance", err)
	}
	...
	return governance, overrideSummary, governanceAuditSummary, nil
}
...
func buildTaskCostOverrideReadModels(
	...
	events []*domain.TaskEvent,
	overrideEvents []*domain.TaskCostOverrideAuditEvent,
) (*domain.TaskCostOverrideSummary, *domain.TaskGovernanceAuditSummary) {
	if len(overrideEvents) > 0 {
		return buildTaskCostOverrideSummaryFromAuditEvents(detail, overrideEvents)
	}
...
func buildTaskCostOverrideSummaryFromAuditEvents(
	...
	return summary, buildTaskGovernanceAuditSummary(detail, summary, len(events))
}

func buildTaskCostOverrideSummaryFromTaskEvents(detail *domain.TaskDetail, events []*domain.TaskEvent) (*domain.TaskCostOverrideSummary, *domain.TaskGovernanceAuditSummary) {
	if detail == nil {
		return nil, nil
	}
	...
	return summary, buildTaskGovernanceAuditSummary(detail, summary, auditEventCount)
}
