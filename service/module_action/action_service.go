...
func NewActionService(tasks repo.TaskRepo, modules repo.TaskModuleRepo, events repo.TaskModuleEventRepo, refs repo.ReferenceFileRefFlatRepo, txRunner repo.TxRunner, rules *blueprint.RuleEngine, opts ...Option) *ActionService {
	s := &ActionService{tasks: tasks, modules: modules, events: events, refs: refs, txRunner: txRunner, rules: rules}
...
func (s *ActionService) Apply(ctx context.Context, req ActionRequest) permission.Decision {
	...
	isRetouchSubmit := req.ModuleKey == domain.ModuleKeyRetouch && req.Action == domain.ModuleActionSubmit
	if !isCustomizationSubmit && !isRetouchSubmit {
		return permission.Deny(domain.DenyModuleActionRoleDenied, "module action is not part of the active workflow")
	}
	...
	} else if task.TaskType != domain.TaskTypeRetouchTask || task.TaskStatus != domain.TaskStatusInProgress {
		return permission.Deny(domain.ErrCodeInvalidStateTransition, "修图任务当前不可提交")
	} else if !domain.EffectiveAccessAllowsTask(req.Actor, domain.PermissionTaskDesignSubmit, task.AccessSubject()) {
		return permission.Deny(domain.ErrCodePermissionDenied, "task.design.submit is required in the task scope")
	}
	...
	if tm.ClaimedBy == nil || req.Actor.ID <= 0 || *tm.ClaimedBy != req.Actor.ID {
		return permission.Deny(domain.ErrCodePermissionDenied, "only the current module handler can submit")
	}
...
func eventTypeForAction(action string) domain.ModuleEventType {
	if action == domain.ModuleActionSubmit {
