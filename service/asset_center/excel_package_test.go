...
func TestExcelPackageAcceptsTIFAndRejectsPSD(t *testing.T) {
	tifMime := "image/tiff"
	psdMime := "image/vnd.adobe.photoshop"
	if !isExcelPackageImage(&domain.TaskAsset{FileName: "HSC34548.tif", MimeType: &tifMime}) {
		t.Fatal("expected TIF production image to be accepted")
	}
	if !isExcelPackageExternalImage(&AssetDetail{FileName: "GK000804.tiff", MimeType: tifMime}) {
		t.Fatal("expected external TIFF production image to be accepted")
	}
	if isExcelPackageImage(&domain.TaskAsset{FileName: "source.psd", MimeType: &psdMime}) {
		t.Fatal("expected PSD source file to remain excluded from warehouse image packaging")
	}
}
