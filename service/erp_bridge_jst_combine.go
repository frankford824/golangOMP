...
func jstMapToCombineSKUItem(row map[string]interface{}) domain.JSTCombineSKUItem {
	...
	item := domain.JSTCombineSKUItem{
		...
		EntitySKUID:  firstString(row, "enty_sku_id", "entity_sku_id"),
		PicURL:       firstString(row, "pic", "pic_url", "image_url", "img_url"),
		Brand:        firstString(row, "brand"),
		VCName:       firstString(row, "vc_name"),
		Properties:   firstString(row, "properties_value"),
		...
		Weight:       firstFloatPtr(row, "weight"),
		SKUQty:       firstFloatPtr(row, "sku_qty"),
		ERPCreatedAt: jstTimePtr(firstString(row, "created", "created_at", "create_time")),
