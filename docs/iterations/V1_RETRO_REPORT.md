...
| 优先级 | 项 | 证据 |
|---|---|---|
| **CRITICAL** | `transport/handler/task_detail.go` GetByTaskID fallback 出口残留(L44-49 调 `h.svc.GetByTaskID` 返回 36 字段老 `domain.TaskDetailAggregate`)· 与 OpenAPI 5 字段 schema 不符 · fast-path init 失败将引发前端崩 | `V1_2_CONTRACT_AUDIT_v2.json` GET `/v1/tasks/:id/detail` verdict=both_diff |
