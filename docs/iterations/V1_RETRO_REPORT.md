...
| 优先级 | 项 | 证据 |
|---|---|---|
| **CRITICAL · CLOSED V1.2-D-1** | `transport/handler/task_detail.go` GetByTaskID fallback 出口已切除;构造改为 `NewTaskDetailHandler(r3DetailSvc)`, `r3Svc == nil` 返回 internal error,成功路径唯一返回 5 段 `task_aggregator.Detail` | `docs/iterations/V1_2_D_1_REPORT.md`; audit after: GET `/v1/tasks/:id/detail` verdict=clean, drift 72→71 |
