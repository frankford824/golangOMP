...
- 本文件覆盖 `162` 个 `/v1` path；同一路径多 method 合并在同一节。
...
- `GET`: Returns versioned, administrator-maintained reason tags. When the experience UI flag is off, the backend returns an empty list without reading the new tables.
...
| 参数 | 位置 | 类型 | 必填 | 说明 |
|---|---|---|---|---|
| `scene` | query | string | 否 | Optional tag scene filter. |
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `data` | array<ExperienceReasonTag> | 是 | - |
...
- `GET`: Unified manager search across system assets, asset workbench submissions, and piecework items. Date filters use Beijing business-day input when YYYY-MM-DD is supplied.
...
- `GET`: Client-visible upload destinations. When this list is non-empty, upload session creation requires `upload_directory_id`.
