...
| GET | `/v1/cost-rule-bindings/skus` | Browse SKUs by current active style binding | GET:已登录 / scope-aware | [V1_API_TASKS.md](V1_API_TASKS.md) |
...
| POST | `/v1/tasks/{id}/references/replace` | Replace one displayed task-level reference with a newly uploaded reference | POST:已登录 / scope-aware | [V1_API_TASKS.md](V1_API_TASKS.md) |
...
| GET | `/v1/task-board/design-department` | Get the caller's department design performance dashboard | GET:已登录 / scope-aware | [V1_API_TASKS.md](V1_API_TASKS.md) |
...
| GET | `/v1/integration/asset-sync/external-current/head` | Capture the external-current change-feed high-water cursor | GET:已登录 / scope-aware | [V1_API_TASKS.md](V1_API_TASKS.md) |
| GET | `/v1/integration/asset-sync/external-current/changes` | Long-poll the low-latency external-current change feed | GET:已登录 / scope-aware | [V1_API_TASKS.md](V1_API_TASKS.md) |
...
| GET, POST | `/v1/asset-workbench/error-imports` | List quality error import batches；Import asset workbench quality error deductions | GET:AssetSettlement, SuperAdmin; POST:AssetManager, AssetSettlement, SuperAdmin | [V1_API_TASKS.md](V1_API_TASKS.md) |
...
| GET, DELETE | `/v1/asset-workbench/error-imports/{import_id}` | Get one quality error import batch with row details；Delete one quality error import batch and all its rows | GET:AssetSettlement, SuperAdmin; DELETE:AssetSettlement, SuperAdmin | [V1_API_TASKS.md](V1_API_TASKS.md) |
...
| POST | `/v1/asset-workbench/settlement/batches/{batch_id}/reverse-confirmation` | Reverse one confirmed but unpaid settlement batch | POST:SuperAdmin | [V1_API_TASKS.md](V1_API_TASKS.md) |
...
| POST | `/v1/assets/excel-package/jobs` | Create a durable server-side production ZIP job from normalized rows | POST:已登录 / scope-aware | [V1_API_ASSETS.md](V1_API_ASSETS.md) |
| POST | `/v1/assets/excel-package/jobs/file` | Parse XLS or XLSX and create a durable server-side production ZIP job | POST:已登录 / scope-aware | [V1_API_ASSETS.md](V1_API_ASSETS.md) |
| GET | `/v1/assets/excel-package/jobs/{job_id}` | Read the current user's production package job | GET:已登录 / scope-aware | [V1_API_ASSETS.md](V1_API_ASSETS.md) |
