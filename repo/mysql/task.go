...
func (r *taskRepo) List(ctx context.Context, filter repo.TaskListFilter) ([]*domain.TaskListItem, int64, error) {
	...
	query := fmt.Sprintf(`
		...
		WHERE %s
		ORDER BY %s
		LIMIT ? OFFSET ?`, spec.fromSQL, taskActorNameJoins(), spec.whereSQL, taskListOrderBy(filter.Sort))
...
func scanTaskListItemRow(rows *sql.Rows) (*domain.TaskListItem, error) {
	...
	var primarySKUCode sql.NullString
	var businessLane sql.NullString
	var costRuleID, matchedRuleVersion sql.NullInt64
	var costPrice, estimatedCost sql.NullFloat64
	...
	if err := rows.Scan(
		...
		&item.TaskStatus, &item.CreatedAt, &item.UpdatedAt, &deadlineAt, &businessLane, &item.CustomizationRequired,
...
func buildTaskListQuerySpecWithOptions(filter repo.TaskListFilter, candidateFilters []domain.TaskQueryFilterDefinition, options taskListQueryBuildOptions) (taskListQuerySpec, error) {
	...
	if err := appendTaskQueryDefinitionWhere(&where, &args, filter.TaskQueryFilterDefinition); err != nil {
		return taskListQuerySpec{}, err
	}
	if clause, clauseArgs, err := buildTaskBoardCandidateScopeClause(candidateFilters); err != nil {
		return taskListQuerySpec{}, err
	} else if clause != "" {
...
func appendTaskQueryDefinitionWhere(where *[]string, args *[]interface{}, filter domain.TaskQueryFilterDefinition) error {
...
func buildTaskBoardCandidateScopeClause(candidateFilters []domain.TaskQueryFilterDefinition) (string, []interface{}, error) {
	...
	for _, candidate := range candidateFilters {
		...
		if err := appendTaskQueryDefinitionWhere(&candidateWhere, &candidateArgs, candidate); err != nil {
			return "", nil, err
		}
