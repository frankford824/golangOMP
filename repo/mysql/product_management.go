...
const productManagementSelectCols = `
	id, record_key, task_id, task_sku_item_id, task_no, sku_code, product_i_id,
	...
	image_filename, image_mime_type, image_missing_reason, erp_sync_status,
	last_erp_checked_at, last_erp_synced_at, sync_cooldown_until, last_sync_error,
...
func buildProductManagementWhere(filter repo.ProductManagementListFilter) (string, []interface{}) {
	...
	if keyword != "" {
		...
		args = append(args, like, like, like, like, like, keyword)
	}
	...
	if strings.TrimSpace(filter.IssueScope) == "attention" {
		clauses = append(clauses, `(
			...
			OR cost_price <= 0
			OR erp_sync_status IN ('pending_sync', 'failed', 'queued', 'cooling_down', 'syncing')
		)`)
	}
...
func scanProductManagementRecordScanner(scanner productManagementScanner) (*domain.ProductManagementRecord, error) {
	...
	var lastERPCheckedAt, lastERPSyncedAt, syncCooldownUntil sql.NullTime
	if err := scanner.Scan(
		&item.ID,
		&item.RecordKey,
		&item.TaskID,
