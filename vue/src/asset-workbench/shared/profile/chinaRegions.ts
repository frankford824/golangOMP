...
export const chinaRegionOptions: readonly ChinaRegionOption[] = [
  ...
  { province: '青海', cities: ['西宁', '海东', '海北', '黄南', '海南州', '果洛', '玉树', '海西'] },
  { province: '宁夏', cities: ['银川', '石嘴山', '吴忠', '固原', '中卫'] },
  { province: '新疆', cities: ['乌鲁木齐', '克拉玛依', '吐鲁番', '哈密', '昌吉', '博尔塔拉', '巴音郭楞', '阿克苏', '克孜勒苏', '喀什', '和田', '伊犁', '塔城', '阿勒泰', '石河子', '阿拉尔', '图木舒克', '五家渠', '北屯', '铁门关', '双河', '可克达拉', '昆玉', '胡杨河', '新星'] },
  { province: '香港', cities: ['香港特别行政区'] },
  { province: '澳门', cities: ['澳门特别行政区'] },
  { province: '台湾', cities: ['台北', '新北', '桃园', '台中', '台南', '高雄', '基隆', '新竹', '嘉义'] },
]

export function provinceOptions(current = ''): string[] {
  const options = chinaRegionOptions.map((item) => item.province)
  const value = current.trim()
  return value && !options.includes(value) ? [value, ...options] : options
}

export function cityOptions(province: string, current = ''): string[] {
  const options = [...(chinaRegionOptions.find((item) => item.province === province)?.cities ?? [])]
  const value = current.trim()
  return value && !options.includes(value) ? [value, ...options] : options
}
