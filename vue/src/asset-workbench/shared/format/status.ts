...
export function eventTypeMeta(value?: string | null): StatusMeta {
  ...
  const known: Record<string, StatusMeta> = {
    ...
    'file.moved': { label: '移动文件', tone: 'info' },
    'file.deleted': { label: '删除文件', tone: 'danger' },
    'file.renamed': { label: '重命名文件', tone: 'neutral' },
    'group.upserted': { label: '维护人员分组', tone: 'info' },
    'template.upserted': { label: '历史模板维护', tone: 'info' },
    'template.assigned': { label: '历史模板下发', tone: 'neutral' },
    'template_assignment.removed': { label: '历史模板撤销', tone: 'neutral' },
    'member.identity_changed': { label: '调整成员功能', tone: 'info' },
    ...
    profile_update: { label: '资料维护', tone: 'info' },
  ...
  return lookup(
    {
      ...
      template: { label: '历史模板', tone: 'neutral' },
      template_assignment: { label: '历史模板下发', tone: 'neutral' },
      member: { label: '成员管理', tone: 'info' },
...
export function eventReasonText(value?: string | null) {
  ...
  const known: Record<string, string> = {
    ...
    'create template': '创建历史模板',
    'update template': '更新历史模板',
    'disable template': '停用历史模板',
    'assign template': '历史模板下发',
    'remove assignment': '撤销历史模板下发',
    'workbench role update': '调整可用功能',
    'merge account': '合并账号',
