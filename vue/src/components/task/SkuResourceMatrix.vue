<template>
  <section class="resource-matrix" aria-labelledby="resource-matrix-title">
      ...
      <div class="stage-rail" aria-label="资源三个阶段">
        <section class="stage-card reference-stage">
          ...
          <div v-if="references(group).length" class="reference-grid">
            <button v-for="(reference, index) in references(group)" :key="reference.id || index" type="button" class="visual-tile" :disabled="!reference.preview_url || previewFailed(previewKey(group.id, 'reference', reference.id || index))" @click="openPreview(reference.preview_url, reference.file_name || `参考图 ${index + 1}`)">
        ...
        <section class="stage-card final-stage">
          ...
          <div v-if="orderedItems(group).length" class="final-gallery">
            <button v-for="(item, index) in orderedItems(group)" :key="item.id" type="button" class="final-tile" :disabled="!item.file?.preview_url || previewFailed(previewKey(group.id, 'final', item.id))" @click="openPreview(item.file?.preview_url, item.file?.file_name || item.item_name || `成品 ${index + 1}`)">
    ...
    <Teleport to="body">
      <div v-if="preview.url" class="preview-layer" role="dialog" aria-modal="true" :aria-label="preview.name" @click.self="closePreview" @keydown.esc="closePreview">
...
import type { ResourceBundle, ResourceGroup, ResourceReference, ResourceRevision } from '@/services/api/resourceGroupsApi'
