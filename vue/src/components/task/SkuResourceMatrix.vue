            ...
            <template v-for="(reference, index) in references(group)" :key="reference.id || index">
              <button v-if="imagePreviewable(reference)" type="button" class="visual-tile" :disabled="!reference.preview_url" @click="openResourcePreview(reference.formal_task_asset_id, reference.preview_url, reference.file_name || `参考图 ${index + 1}`)">
                <AssetPreviewMedia
                  ...
                  class="controlled-preview"
                  :task-asset-id="reference.formal_task_asset_id ? String(reference.formal_task_asset_id) : null"
                  :fallback-src="reference.preview_url"
                  :alt="reference.file_name || `参考图 ${index + 1}`"
                  img-class="controlled-preview-shell"
                  inner-img-class="controlled-preview-img"
                  @open-full="(url) => openPreview(url, reference.file_name || `参考图 ${index + 1}`)"
                />
        ...
        <section class="stage-card final-stage">
          ...
          <div v-if="orderedItems(group).length" class="final-gallery">
            <template v-for="(item, index) in orderedItems(group)" :key="item.id">
              <button v-if="imagePreviewable(item.file)" type="button" class="final-tile" :disabled="!item.file?.preview_url" @click="openResourcePreview(item.task_asset_id, item.file?.preview_url, item.file?.file_name || item.item_name || `成品 ${index + 1}`)">
                <AssetPreviewMedia
                  v-if="item.file?.preview_url"
                  class="controlled-preview"
                  :task-asset-id="String(item.task_asset_id)"
                  :fallback-src="item.file.preview_url"
                  :alt="item.file.file_name"
                  img-class="controlled-preview-shell"
                  inner-img-class="controlled-preview-img"
                  @open-full="(url) => openPreview(url, item.file?.file_name || item.item_name || `成品 ${index + 1}`)"
                />
...
import { materializePreviewImageUrl, revokeMaterializedPreviewImage, type MaterializedPreviewImage } from '@/domain/asset-preview-image'
...
let activePreviewImage: MaterializedPreviewImage | null = null
...
function releaseActivePreview() {
  revokeMaterializedPreviewImage(activePreviewImage)
...
function openPreview(url?: string, name?: string) {
  ...
  preview.url = url
  preview.name = name || '资源预览'
}
async function openResourcePreview(taskAssetID?: number | null, fallbackURL?: string, name?: string) {
  const id = taskAssetID ? String(taskAssetID) : ''
  let controlledURL = String(fallbackURL || '').trim()
  if (id) {
    const meta = await fetchTaskAssetPreviewMeta(id)
    if (meta.status !== 'ok' || !meta.displayUrl) return
    controlledURL = meta.displayUrl
  }
  if (!controlledURL) return
  const image = await materializePreviewImageUrl(controlledURL, id)
  if (!image) return
  releaseActivePreview()
  activePreviewImage = image
  preview.url = image.displaySrc
  preview.name = name || '资源预览'
}
function closePreview() { releaseActivePreview(); preview.url = ''; preview.name = '' }
onBeforeUnmount(releaseActivePreview)
</script>
