<template>
  <Teleport to="body">
    <div class="revision-drawer-backdrop" :style="{ zIndex: RESOURCE_OVERLAY_Z_INDEX }" @mousedown.self="emit('close')">
          ...
          <article v-for="revision in result.items" :key="revision.id" class="revision-card">
            ...
            <section v-if="revision.legacy_migration" class="migration-evidence" aria-label="历史迁移证据">
              ...
              <template v-if="revision.evidence_summary">
                <dl>
                  ...
                  <div v-if="!revision.evidence_summary.business_reason && revision.evidence_summary.business_reason_sha256">
                    <dt>业务理由摘要</dt>
                    <dd><code>{{ shortHash(revision.evidence_summary.business_reason_sha256) }}</code></dd>
                  </div>
                  <div><dt>上传会话</dt><dd>{{ uploadSessionLabel(revision) }}</dd></div>
                </dl>
                <p v-if="revision.evidence_summary.evidence_event_ids_complete">
                  事件证据（完整 {{ revision.evidence_summary.evidence_event_count }} 条）：{{ revision.evidence_summary.evidence_event_ids.join('、') }}
                </p>
                <p v-else>
                  事件证据共 {{ revision.evidence_summary.evidence_event_count }} 条；当前仅保留首条 {{ firstEvidenceEventID(revision) }}。完整集合请按清单摘要 {{ shortHash(revision.evidence_summary.manifest_sha256) }} 查询迁移清单。
                </p>
              </template>
            ...
            <section class="revision-files">
              ...
              <div v-if="revision.source_file" class="file-row">
                ...
                <div>
                  <strong>{{ revision.source_file.file_name }}</strong>
                  <small v-if="isHistoricalUnavailable(revision.source_file)" class="file-unavailable">{{ historicalUnavailableLabel(revision.source_file) }}</small>
                  <small v-else>{{ formatBytes(revision.source_file.file_size) }}</small>
                </div>
                <nav>
            ...
            <section class="revision-files">
              ...
              <div v-if="revision.items.length" class="file-grid">
                <article v-for="(item,index) in orderedItems(revision)" :key="item.id">
                  ...
                  <div>
                    <strong>{{ item.file?.file_name || item.item_name || `成品 ${index + 1}` }}</strong>
                    <small v-if="item.file && isHistoricalUnavailable(item.file)" class="file-unavailable">{{ historicalUnavailableLabel(item.file) }}</small>
                    <small v-else>顺序 {{ index + 1 }}</small>
                  </div>
            ...
            <section v-if="revision.references.length" class="revision-files">
              ...
              <div class="reference-list">
                <div v-for="reference in orderedReferences(revision)" :key="reference.id || reference.reference_file_ref_id">
                  <span>
                    {{ reference.file_name || reference.ref_id || '参考资料' }}
                    <small v-if="isHistoricalUnavailable(reference)" class="file-unavailable">{{ historicalUnavailableLabel(reference) }}</small>
                  </span>
                  <nav>
...
import { resourceGroupsApi, type ResourceFile, type ResourceGroup, type ResourceRevision, type ResourceRevisionListResult } from '@/services/api/resourceGroupsApi'
...
function displayReason(revision: ResourceRevision) {
  const evidenceReason = revision.evidence_summary?.business_reason
  const rawReason = evidenceReason || revision.reason || ''
  const markerIndex = rawReason.lastIndexOf('[migration_v2 ')
  const businessReason = markerIndex >= 0 ? rawReason.slice(0, markerIndex).trim() : rawReason
  if (revision.evidence_summary?.confidence !== 'confirmed_auto') return businessReason
  return businessReason
    .replace(/\s*[;；]\s*human confirmation remains required\.?/g, '')
    .replace(/human confirmation remains required\.?/g, '')
    .trim()
}
...
function firstEvidenceEventID(revision: ResourceRevision) { return revision.evidence_summary?.evidence_event_ids[0] || '未保留' }
...
function isHistoricalUnavailable(file: Pick<ResourceFile, 'availability'>) { return file.availability === 'historical_unavailable' }
function historicalUnavailableLabel(file: Pick<ResourceFile, 'unavailable_reason'>) {
  return file.unavailable_reason === 'legacy_original_object_missing'
    ? '历史文件不可用（原始对象已缺失）'
    : '历史文件不可用'
}
...
async function previewAsset(assetId: number) {
  actionError.value = ''
  const meta = await fetchTaskAssetPreviewMeta(String(assetId))
...
async function downloadAsset(assetId: number, fileName?: string) {
  actionError.value = ''
  const outcome = await downloadAssetFileWithOriginalFilename({ taskAssetId: String(assetId), preferredFilename: fileName })
