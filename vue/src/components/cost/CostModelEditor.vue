<template>
  <section class="model-editor" aria-label="统一计价方案">
    <div class="fields">
      <label v-if="!hideMaterial">材质<input v-model.trim="draft.material" @input="update" placeholder="例如：覆膜KT板" /></label>
      ...
      <template v-if="draft.basis !== 'manual'">
        <label>每{{draft.basis==='area'?'平方米':draft.basis==='piece'?'件':'套'}}价格（元）<input v-model.number="draft.unit_price" required type="number" min="0" step="any" @input="update" /></label>
        <details class="extras" :open="draft.multiplier!==1 || draft.minimum>0 || draft.small_area_threshold>0"><summary>起步价与价格调整（选填）</summary><div class="fields">
        <label>价格调整倍数<input v-model.number="draft.multiplier" type="number" min="0.0001" step="any" @input="update" /></label>
    ...
    <p class="formula">{{draft.basis==='manual'?'每个产品由工作人员确认价格。':'按实际用料收费，选中的加工项目另计费用。'}}</p>
    <template v-if="draft.basis !== 'manual'">
      <details :open="draft.thickness_prices.length>0"><summary>不同厚度，不同价格（选填）</summary>
...
const props=defineProps<{modelValue:string;hideMaterial?:boolean}>();const emit=defineEmits<{'update:modelValue':[string]}>()
