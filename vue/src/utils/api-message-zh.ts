...
export const API_ERROR_MESSAGE_ZH: Record<string, string> = {
  ...
  'upload_session is already terminal': '该文件的上传会话已取消或已过期，请重新选择这个文件上传；其他已成功文件无需重传',
  'upload_session changed concurrently and is already terminal': '该文件的上传会话状态已变化，请重新选择这个文件上传；其他已成功文件无需重传',
  ...
  'settlement batch is not confirmation-reversible.': '只有已确认批次可以撤销确认，请刷新后查看最新状态。',
  'settlement batch has adjustments and cannot be confirmation-reversed.': '该批次已有工资调整记录，为保护账务链路不能撤销确认。请继续使用工资调整处理差额。',
  'expected_batch_no does not match the confirmed settlement batch.': '输入的批次号与当前已确认批次不一致，请重新核对。',
  'confirm_unpaid must be true before reversing a confirmed settlement batch.': '撤销前必须确认该批次尚未实际发放。已发放批次只能使用工资调整。',
  'only super administrators can reverse confirmed settlement batches.': '只有超级管理员可以撤销已确认批次。',
  'submission item cannot be changed after settlement batch attachment.': '当前作品已关联结算批次，不能移动或删除。待确认批次请先取消；已确认批次需由超级管理员执行受控撤销确认。',
  'submission item cannot be edited after settlement or void.': '当前作品已进入结算或已作废，不能再修改。',
  'all files in one priced work must be selected together.': '该作品由文件夹内的多个文件组成，请勾选整个文件夹作品后再移动或删除。',
  'failed to load all files in the priced work.': '暂时无法读取该作品的全部文件，请刷新后重试。',
