<template>
  <main class="task-page task-detail-view">
    ...
    <template v-else-if="task">
      ...
      <section v-if="isPlanning" class="planning-results" aria-label="策划 SKU 明细">
        <header>
          <div>
            <p class="eyebrow">策划结果</p>
            <h2>SKU 明细与产品图片</h2>
            <p>产品图片属于策划 SKU 修订，不会重复记入普通任务附件。</p>
          </div>
          <strong>{{ planningItems.length }} 个 SKU</strong>
        </header>
        <div v-if="planningItems.length" class="planning-result-grid">
          <article v-for="item in planningItems" :key="item.task_sku_item_id">
            <div class="planning-image">
              <img
                v-if="item.revision?.product_image_url"
                :src="item.revision.product_image_url"
                :alt="item.revision.product_image_name || `${item.sku_code} 产品图片`"
              >
              <span v-else>未上传产品图片</span>
            </div>
            <div class="planning-result-copy">
              <p>第 {{ item.sequence_no }} 项</p>
              <h3>{{ item.sku_code }}</h3>
              <strong>{{ item.revision?.description_spec || '未填写产品描述' }}</strong>
              <dl>
                <div><dt>数量</dt><dd>{{ item.quantity }}</dd></div>
                <div><dt>目标价</dt><dd>{{ item.revision?.target_price ? `¥${item.revision.target_price}` : '—' }}</dd></div>
                <div><dt>图片</dt><dd>{{ item.revision?.product_image_name || '无' }}</dd></div>
              </dl>
            </div>
          </article>
        </div>
        <p v-else class="muted-copy">当前任务没有可展示的策划 SKU 明细。</p>
      </section>
          ...
          <div v-else-if="workspaceMode === 'details'" class="workspace-body detail-sections">
            ...
            <section v-if="isPlanning" class="planning-detail-section">
              ...
              <div class="planning-detail-list">
                <article v-for="item in planningItems" :key="item.task_sku_item_id">
                  <img v-if="item.revision?.product_image_url" :src="item.revision.product_image_url" :alt="item.revision.product_image_name || `${item.sku_code} 产品图片`">
                  <span v-else class="planning-image-empty">无图片</span>
                  <div><strong>{{ item.sku_code }}</strong><p>{{ item.revision?.description_spec || '未填写产品描述' }}</p><small>数量 {{ item.quantity }} · 图片 {{ item.revision?.product_image_name || '无' }}</small></div>
                </article>
              </div>
            </section>
...
const planningItems = computed(() => planningResult.value?.items || [])
...
.detail-sections{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));grid-auto-rows:max-content;align-content:start;gap:13px}.detail-sections>section{padding:16px;border:1px solid rgb(var(--yb-border));border-radius:15px}.detail-summary-strip,.detail-requirement,.sku-item-section,.planning-detail-section,.edit-attachment-hint{grid-column:1/-1}.edit-attachment-hint{display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:13px;background:rgb(var(--yb-surface-soft))}.edit-attachment-hint>div:first-child{min-width:min(100%,26ch);flex:1 1 26ch;display:grid;gap:4px}.edit-attachment-hint strong{font-size:14px}.edit-attachment-hint p{margin:0;color:rgb(var(--yb-text-secondary));font-size:12px;line-height:1.6}.edit-attachment-actions{display:flex;flex-wrap:wrap;gap:8px}.planning-detail-list{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:9px;margin-top:10px}.planning-detail-list article{display:grid;grid-template-columns:64px minmax(0,1fr);gap:10px;padding:9px;border:1px solid rgb(var(--yb-border));border-radius:11px;background:rgb(var(--yb-surface-soft))}.planning-detail-list img,.planning-image-empty{width:64px;height:64px;border-radius:9px;object-fit:cover}.planning-image-empty{display:grid;place-items:center;background:rgb(var(--yb-surface-muted));color:rgb(var(--yb-text-muted));font-size:10px}.planning-detail-list p{margin:4px 0;color:rgb(var(--yb-text-body))}.planning-detail-list small{color:rgb(var(--yb-text-muted))}.detail-sections>.detail-summary-strip{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:1px;padding:0;overflow:hidden;background:rgb(var(--yb-border))}.detail-summary-strip div{padding:13px;background:rgb(var(--yb-surface-soft))}.detail-summary-strip span{display:block;color:rgb(var(--yb-text-muted));font-size:10px}.detail-summary-strip strong{display:block;margin-top:3px}.detail-copy-grid{display:grid;grid-template-columns:1.15fr .85fr;gap:13px}.detail-copy-grid>aside{padding:13px;border-radius:12px;background:rgb(var(--yb-brand-soft))}.detail-sections h3{margin:9px 0 5px}.sku-item-heading{display:flex;align-items:flex-end;justify-content:space-between;gap:12px}.sku-item-heading h3{margin:3px 0 0}.sku-item-heading>span{color:rgb(var(--yb-text-muted));font-size:11px}.long-copy{margin:0;white-space:pre-wrap;color:rgb(var(--yb-text-body));line-height:1.75}.detail-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1px;margin:10px 0 0;overflow:hidden;border:1px solid rgb(var(--yb-border));border-radius:11px;background:rgb(var(--yb-border))}.detail-list div{padding:10px;background:rgb(var(--yb-surface))}.detail-list dt{font-size:10px;color:rgb(var(--yb-text-muted))}.detail-list dd{margin:3px 0 0}
...
/* 面板自己管内部滚动，但之前没被撑满弹层高度，内容超高就直接被裁掉且没有滚动条。
   撑满后由面板内部视口滚动；min-height 兜不住的残余情况再由这里滚。 */
.workflow-body{display:grid;grid-template-rows:minmax(0,1fr);overflow:auto}
