...
func registerV1AdminRoutes(
	...
	v1.GET("/users/designers", access(v1, http.MethodGet, "/users/designers", domain.APIReadinessReadyForFrontend, domain.RoleOps, domain.RoleDesigner, domain.RoleCustomizationOperator, domain.RoleAuditA, domain.RoleAuditB, domain.RoleAdmin, domain.RoleHRAdmin, domain.RoleSuperAdmin, domain.RoleDeptAdmin, domain.RoleTeamLead, domain.RoleDesignDirector), userAdminH.ListDesigners)
	...
	v1.DELETE("/users/:id", access(v1, http.MethodDelete, "/users/:id", domain.APIReadinessReadyForFrontend, domain.RoleSuperAdmin), withDeprecatedRoute("/v1/users/{id}/deactivate", "compatibility_only_no_frontend_entry"), userAdminH.Delete)
